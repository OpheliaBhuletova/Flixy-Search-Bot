...
def _normalize_tmdb_id(query: str) -> Optional[int]:
    q = query.strip().lower()

    if q.isdigit():
        return int(q)

    q = re.sub(r"^tmdb[:\s]+", "", q).strip()
...
async def get_poster(
    ...
    *,
    ...
    tmdb_id = _normalize_tmdb_id(query)
    if id:
        tmdb_id = _normalize_tmdb_id(query)
    ...
    if tmdb_id is not None:
        if preferred_type == "tv":
            details = await _get_tmdb_details(tmdb_id, "tv")
            media_type = "tv"
        elif preferred_type == "movie":
            details = await _get_tmdb_details(tmdb_id, "movie")
            media_type = "movie"
        else:
            details = await _get_tmdb_details(tmdb_id, "movie")
            media_type = "movie"
